...
const config = {
  ...
  // Firebase Realtime Database — the ONLY persistence layer (users, sessions,
  // contacts, jobs, settings) plus the realtime event feed. No local files,
  // no ./data, no PostgreSQL required. FIREBASE_ENABLED=false disables the
  // realtime event mirror (the worker WebSocket hub remains for events).
  firebase: {
    ...
    apiKey: process.env.FIREBASE_API_KEY || 'AIzaSyBAYTZcLWIqvzcpFRDNdXuQjL_rnTgkGnM',
    authDomain: process.env.FIREBASE_AUTH_DOMAIN || 'whatsbotog.firebaseapp.com',
    databaseURL: process.env.FIREBASE_DATABASE_URL || 'https://whatsbotog-default-rtdb.firebaseio.com',
    projectId: process.env.FIREBASE_PROJECT_ID || 'whatsbotog',
    storageBucket: process.env.FIREBASE_STORAGE_BUCKET || 'whatsbotog.firebasestorage.app',
    messagingSenderId: process.env.FIREBASE_MESSAGING_SENDER_ID || '641852211328',
    appId: process.env.FIREBASE_APP_ID || '1:641852211328:web:b8c7e3c194a1538b896665',
  },
