...
/**
 * Secure flag only when the browser actually talks HTTPS — otherwise the
 * session cookie is silently dropped by browsers on HTTP (local, VPS, LAN).
 */
function isHttpsRequest(request) {
  if (!request) return process.env.NODE_ENV === 'production';
  const fwd = String(request.headers?.get('x-forwarded-proto') || '').split(',')[0].trim();
  if (fwd === 'https') return true;
  const proto = request.nextUrl?.protocol || '';
  return proto === 'https:';
}

function setSessionCookie(res, token, request) {
...
function clearSessionCookie(res, request) {
  res.cookies.set(config.cookieName, '', { httpOnly: true, sameSite: 'lax', secure: isHttpsRequest(request), maxAge: 0, path: '/' });
  return res;
}
