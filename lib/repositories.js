...
const { normalizePhone, formatPhone, isValidPhone, cleanName, validateName } = require('./phone');
...
const contactsRepo = {
  ...
  async updatePhone(id, phone) {
    const normalizedPhone = normalizePhone(phone);
    if (!normalizedPhone || !isValidPhone(normalizedPhone)) return { ok: false, reason: 'Yanlış nömrə formatı' };
    const row = await this.getById(id);
    if (!row) return { ok: false, reason: 'Kontakt tapılmadı' };
...
const usersRepo = {
  ...
  /** Password-only verification — anyone with the right password can log in. */
  async verify(_username, password) {
  ...
  async changePassword(currentPassword, newPassword) {
    ...
    await fb.set(`${P.users}/${ADMIN_KEY}`, { ...row, passwordHash: `${salt}:${hash}` });
